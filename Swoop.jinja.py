...
representation data structure that represents the file's contents and
provides simple accessors and mutators to read and modify those contents, and
generates valid Eagle files as output.

Eagle files are stored as XML and are, therefore, tree-structured.  
The Swoop data structures are also tree-structured and closely resemble the Eagle file's
...
The classes that Swoop uses fall into several categories:
 ...
 5. **Exception classes** For reporting errors.

 6. **Utility classes**  Most notably :class:`Flock` which provides Swoop with a fluent interface for accessing and iterating over :class:`EagleFilePart` objects and :class:`EagleFilePartVisitor` which implements a visitor framework Swoop data structures.

The code for most of Swoop is autogenerated by a Python script called :code:`GenerateSwoop.py` using the Jinja templating system.
...
"""
        ...
        if not self.validate():
            ...
            raise SwoopError("element tree does not validate" + str(EagleFile.DTD.error_log.filter_from_errors()[0]))
        else:
            f = open(filename, "w")
            f.write(header+ET.tostring(ET.ElementTree(self.get_et()),pretty_print=True))
...
class Flock(object):
    ...
    def unpack(self):
        """
        Return the contents of the :class:`Flock` object as a list.
        
        :returns: A list containing the contents of the :class:`Flock`
        :rtype:  List
        """
        return self.efps
    ...
    def filter(self, func):
        ...
        :returns: A :class:`Flock` object contain the elements, :code:`x` for which :code:`func(x)` return :code:`True`
        """
        return Flock([x for x in self.efps if func(x)])

    def unique(self):
        """
        Remove duplicate elements from the :class:`Flock`
        
        :returns: A :class:`Flock` object contain the unique elements in this :class:`Flock`.
        """
        return Flock(list(set(self.efps)))

    def count(self):
        """
        Count the elements in the :class:`Flock`
    ...
    def sort(self, cmp=None, key=None, reverse=False):
        """
        Sort the :class:`Flock` object.  Similar to the builtin :code:`sorted` method (and takes the same arguments).

        :param cmp: Comparison function.
        :param key: A key accessor function.
        :param reverse: If :code:`True` sort in reverse order.
        
        :returns: A sorted :class:`Flock`
        """
        return Flock(sorted(self.efps, cmp, key, reverse))

    def map(self, func):
        """
        Apply a function to the elments of this :class:`Flock` and return a :class:`Flock` containing the results.  Similar to the builtin :code:`map` function.
        
        :param func: The function to apply.
        :returns: A :class:`Flock` containing the results.
        """
        return Flock(map(func, self.efps))
    
    def reduce(self, func, init=None):
        """
        Reduce the elments of this :class:`Flock` and return the result.  Similar to the builtin :code:`reduce` function.
...
class {{classname}}({{tag.baseclass}}):
    ...
    def get_{{l.name}}(self):
        """Return (and possibly filter) items in the the :code:`{{l.name}}` map of {{l.get_contained_type_list_doc_string("or")}} objects for this :class:`{{tag.classname}}`.
