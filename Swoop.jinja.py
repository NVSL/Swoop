...
The classes that Swoop uses fall into several categories:
 ...
 6. **Utility classes**  Most notably :class:`From` which provides Swoop with a fluent interface for accessing and iterating over :class:`EagleFilePart` objects and :class:`EagleFilePartVisitor` which implements a visitor framework Swoop data structures.

In addition, Swoop provides a :doc:`several mechanisms  </ExtendingSwoop>` for extending its functionality. 

Because the Swoop data structures map (almost) directly to Eagle file structures, some understanding of the Eagle file format is necessary in order to use Swoop.  However, it is possible to use Swoop without understanding all the details the format.
